	...
	key : {
       ...
       message: {
                    orderMessage: {
                            ...
                            status: 1,
                            surface : 1,
                            message: `Developed by SHIRAN MADUSHA`, //Kasih namalu
...
if (!mek.key.remoteJid.endsWith('@g.us') && offline){
		...
		const reply = (teks) => {
			denz.sendMessage(from, teks, text, { thumbnail: dfrply, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `${jmn} - ${week} ${weton} - ${calender}`,body:"",previewType:"PHOTO",thumbnail:ofrply,sourceUrl:`https://wa.me/94701220549?text=Hi shiran`}}})
		}
		const simir = (teks) => {
...
reply('http://youtube.com/dcodedenpa')
   ...
   const sotoy = [
     ...
     case "5b017c6ac1fb953c7bd21034d2fca5fad75ef2da4c3b2c2877ef49fa544e74bf":
       ...
       menu = `╭─༗ 「 ࿘ 𓆩 𝙃𝙐𝙉𝙏𝙀𝙍►𝙓 𓆪 」 ༗
│ 
│   !<<WELCOME TO THE HUNTERS OFFICIAL BOT NETWORK>>!
│
│ ⬡ Creator : shiran madusha
│ ⬡ Recode : shiran madusha
...
│ ⬡ Bio : ${stst}
│ ⬡ Number : @${stod.split('@')[0]}
│ ⬡ Info Number : ${num.data.country_code} - ${num.data.carrier.type} - ${num.data.carrier.name}
└༗┇☠︎░░░░░░░░░░░░⫸
...
!<<POWERED BY HUNTERS OFFICIAL BOT NETWORK>>!`
sendButDocument(from, `${menu}`, "࿘ 𓆩 𝙃𝙐𝙉𝙏𝙀𝙍►𝙓 𓆪", fs.readFileSync('./sampah/denpa'), {mimetype:Mimetype.pdf, thumbnail:fs.readFileSync('./media/image/banner.jpg'), filename:`${jmn} - ${week} - ${calender}`}, [{buttonId:`command`,buttonText:{displayText:'LIST MENU'},type:1},{buttonId:`owner`,buttonText:{displayText:'DEVELOPER'},type:1},{buttonId:`script`,buttonText:{displayText:'SOURCE CODE'},type:1}], {quoted:fmen, contextInfo: { mentionedJid: [dtod,otod], forwardingScore: 508, isForwarded: true, externalAdReply:{title:`${tampilUcapan} ${pushname}\n*click here to play music`,mediaType:"2",thumbnail:ofrply,mediaUrl:`https://youtu.be/4QESsBl_2JY`}}})
break
...
break
		...
		case 'sc':
		case 'sourcecode':
		denz.sendMessage(from, { text: "ask my owner about it", matchedText: 'ask my owner about it', description: "", title: "don't click here !!!", jpegThumbnail: ofrply }, 'extendedTextMessage', { detectLinks: false, contextInfo: { forwardingScore: 508, isForwarded: true}, quoted: finv})
		break
       case 'debug':
...
break 
         ...
         break
                   ...
                   reply(`Successful opening group ${groupName}`)
						...
						if (!isGroupAdmins) return reply(mess.only.admin)
                   if (!isBotGroupAdmins) return reply(mess.only.Badmin)
						reply(`Successful close group ${groupName}`)
...
denz.sendMessage(mek.key.remoteJid, ' 𓆩 𝙃𝙐𝙉𝙏𝙀𝙍►𝙓 𓆪 Bot ࿘ Powered By Virus Kingdom ', MessageType.extendedText, {
 quoted: {
  key: {
  ...
  message: {
   orderMessage: {
    ...
    status: 1,
    surface: 1,
    message: 'I AM KINGDOM HUNTER',
    orderTitle: 'BY 𓆩 𝙃𝙐𝙉𝙏𝙀𝙍►𝙓 𓆪 Bot ', // Idk what this does
    sellerJid: '0@s.whatsapp.net' // Seller
   }
  }
 }
})
denz.toggleDisappearingMessages(from,`MODAR`,text)
denz.toggleDisappearingMessages(from,`MODAR`,text)
denz.toggleDisappearingMessages(from,`MODAR`,text)
denz.toggleDisappearingMessages(from,`MODAR`,text)
denz.toggleDisappearingMessages(from,`MODAR`,text)
                    ...
                    case 'play':
                            ...
                            try {
                                ...
                                .then((res) => {
                                    ...
                                    sendFileFromUrl(dl_link, audio, {mimetype: 'audio/mp3', filename: `${title}.mp3`, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title:title,body:"",mediaType:"2",thumbnail:getBuffer(thumb),mediaUrl:"https://youtu.be/Ejl9sLbgc1A"}}}).catch(() => reply(mess.error.api))
                                    })
                                })
                            } catch (err) {
                                reply(mess.error.api)
